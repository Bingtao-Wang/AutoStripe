...
| `planning/vision_path_planner.py` | 短期记忆 + 20m距离截断 |
...
1. 感知偶尔失败（右路沿点<2个）时，控制器收到空路径会失控
2. 远距离深度投影噪声大，50m外的路沿点不可靠

### 解决方案

**短期记忆（`vision_path_planner.py`）：**
- 新增 `memory_frames=10` 参数
- 感知成功时保存 `_last_valid_driving` / `_last_valid_nozzle`
- 感知失败时返回上次有效路径，最多持续 10 帧
- 超过 10 帧仍无有效感知则返回空路径

**距离截断：**
- `_sort_by_longitudinal` 增加 `max_range=20.0` 过滤
- 只保留 `0.5 < lon_dist < 20.0` 的路沿点
- 路径点从 ~24 降至 ~14（更稳定）

### 结果

路径点稳定在 14 点，边距读数稳定（喷嘴 2.7-3.0m，追踪点 5.0m）。
...
将感知检测到的右侧路沿用红色点标记显示出来。

### 迭代过程

| 尝试 | 方案 | 结果 |
|------|------|------|
| 1 | 直接用深度投影 z (`pt.z`) | 红点飞到天上（深度 z 不准） |
| 2 | slope 外推 z，无范围限制 | 远处点翘起（50m slope 外推偏差大） |
| 3 | 固定 `veh_loc.z + 0.3` | 平地可以，坡道穿模 |
| 4 | **slope 外推 z + 20m 纵向范围限制** | **正确！与蓝点一致** |

### 最终方案

```python
# 只显示前方 0.5~20m 的点（与 planner 一致）
...
- **横向中位数滤波**：计算所有点横向距离中位数，偏离 >3m 的点丢弃
- **最小间隔 2m**：相邻绘制点距离 <2m 则跳过
- **纵向范围 0.5~20m**：与 planner 截断一致，避免远处 slope 外推偏差
...
蓝点、红点、青色线、绿色线均悬浮在路面上方，视觉上不贴合。

### 修改

所有可视化 z 偏移从 `+0.5` 统一降至 `+0.3`（降低约 20cm）：

| 元素 | 位置 | 修改 |
|------|------|------|
| 蓝色驾驶点 | `draw_driving_path()` | `+0.5` → `+0.3` |
| 红色路沿点 | 主循环可视化 | `+0.5` → `+0.3` |
| 青色追踪点线 | `tp_z` 计算 | `+0.5` → `+0.3` |
| 绿色喷嘴线 | `nozzle_raised` | `+0.5` → `+0.3` |

### 结果

不影响车辆控制（控制系统只用 x,y），视觉更贴合路面。
